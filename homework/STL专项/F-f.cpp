...
统计
提问
URL: http://cxsjsx.openjudge.cn/hw202610/F/
--------------------
*/
...
int main() {
    ...
    cin >> n;
    map<int, int> mem;
    mem.insert(make_pair(100000000, 1));
    for(int i = 0; i < n; ++i) {
        int id, force;
        cin >> id >> force;
        auto it = mem.lower_bound(force);
        int res_id;

        if(mem.count(force) != 0) {
            cout << id << " " << it->second << endl;
            mem[force] = min(id, it->second);
            continue;
        }

        if (it == mem.end()) {
            res_id = prev(it)->second;
        } else if (it == mem.begin()) {
            res_id = it->second;
        } else {
            auto p = prev(it);
            long long diff_next = (long long)it->first - force;
            long long diff_prev = (long long)force - p->first;

            if (diff_prev < diff_next) {
                res_id = p->second;
            }
            else if(diff_next == diff_prev) {
                res_id = min(p->second, it->second);
            } 
            else {
                res_id = it->second;
            }

...
统计
提问
URL: http://cxsjsx.openjudge.cn/hw202610/H/
--------------------
*/
...
template<typename a, typename b, typename c = greater<a> >
class MyMultimap {
public:
	multimap<a, b, c> m;
	using iterator = typename std::multimap<a, b, c>::iterator;
	void insert(pair<a, b> x) {
		m.insert(x);
	}
	auto begin() { return m.begin(); }
	auto end() { return m.end(); }
	void Set(a x, b y) {
		auto [be, en] = m.equal_range(x);
		for(auto it = be; it != en; ++it) {
			it->second = y;
		}
	} 
	auto find(a x) { return m.find(x); }
	void clear() { m.clear(); }
};
